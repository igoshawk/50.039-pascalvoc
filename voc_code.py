...
class VOCDetection(Dataset):
    def __init__(self, root, transform, image_set='train'):
...
def training(model, optimizer, model_name, trainloader):
    model.train()
...
def validating(model, modelname, valloader, t):
    criterion = nn.BCEWithLogitsLoss()
    ...
    with torch.no_grad(): 
        ...
        model.load_state_dict(torch.load(modelname+'.pkl'))
        model.to(device)
        model.eval()
...
def dnn(trainloader, valloader, modelname):
    model = models.resnet18(pretrained = True)
    model.fc = nn.Linear(512,20)
    ...
    training(model, optimizer, modelname, trainloader)
    #best_t = best_threshold(model, optimizer, modelname, trainloader, valloader)
    best_t = 0.4

    validating(model, modelname, valloader, best_t)

    
# predic on a single image
def testing(modelname, path):
    result = []
...
if __name__ == '__main__':
    ...
    dnn(trainloader, valloader, 'voc_model')
    print('Finish training')
    
    testing('model.pkl','data/VOCdevkit/VOC2012/JPEGImages/2007_000039.jpg')
